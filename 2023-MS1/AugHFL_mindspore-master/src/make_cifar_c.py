...
import random
from PIL import Image
from init_data import Cifar10FL
...
train_data = Cifar10FL('./src/dataset', train=True)
convert_img = Compose([py_transforms.ToTensor(), py_transforms.ToPIL()])
...
np.save('./src/dataset/CIFAR-10-C_train/random_noise.npy', np.array(cifar_c).astype(np.uint8))
