...
class JSGNet(nn.Cell):
    def __init__(self, config):
        super(JSGNet, self).__init__()
        self.config = config
        ...
        self.modular_vector_mapping = nn.Dense(config.hidden_size, 1, has_bias=False)


        self.pool_layers = nn.CellList([nn.Identity()]
    ...
    def construct(self, clip_video_feat, frame_video_feat, frame_video_mask, query_feat, query_mask, epoch): # with rec: , words_id, words_feat, words_len, words_weights; per video:, query_labels
        ...
        # matched_easy_neg_score = msnp.diag(sim_based_results['easy_neg_score'])
        # matched_hardest_neg_score = msnp.diag(sim_based_results['hardest_neg_score'])
    ...
    def gauss_weighted_vid_props_feat(self, x_feat):
        ...
        gauss_weight = gauss_weight.unsqueeze(0).broadcast_to((nv, -1, lv))
        proposal_feat_map = ops.bmm(gauss_weight, x_feat)
    ...
    def get_modularized_queries(self, encoded_query, query_mask):
        ...
        modular_attention_scores = ops.softmax(mask_logits(modular_attention_scores, query_mask.unsqueeze(2)), axis=1)
        # modular_queries = ops.einsum("blm,bld->bmd", modular_attention_scores, encoded_query)  # (N, 2 or 1, D)
        modular_queries = ops.bmm(modular_attention_scores.swapaxes(2,1), encoded_query)
        return modular_queries.squeeze()

    @staticmethod
    def get_clip_scale_scores(modularied_query, context_feat):
        l2norm = ops.L2Normalize(axis=-1)
        modularied_query = l2norm(modularied_query)
        context_feat = l2norm(context_feat)

        clip_level_query_context_scores = ops.matmul(context_feat, modularied_query.t()).permute(2, 1, 0)

        query_context_scores, indices = ops.max(clip_level_query_context_scores, # alternative: topk mean
                                                  axis=1)  # (N, N) diagonal positions are positive pairs
    ...
    def get_unnormalized_clip_scale_scores(modularied_query, context_feat):
        ...
        query_context_scores = ops.matmul(context_feat, modularied_query.t()).permute(2, 1, 0)

        query_context_scores, _ = ops.max(query_context_scores, axis=1) # alternative: topk mean

        return query_context_scores

    # new verison, per-query
    def key_clip_guided_attention(self, frame_feat, proposal_feat, feat_mask, max_index):
        selected_max_index = msnp.diag(max_index)
        ...
        if feat_mask is not None:
            scores = ops.bmm(key, query).squeeze()
            masked_scores = scores.masked_fill(feat_mask.equal(0), -1e9).unsqueeze(1)
            masked_scores = nn.Softmax(axis=-1)(masked_scores)
            attention_feat = ops.bmm(masked_scores, value).squeeze()
        else:
            scores = nn.Softmax(axis=-1)(ops.bmm(key, query).swapaxes(1, 2))
            attention_feat = ops.bmm(scores, value).squeeze()
    ...
    def key_clip_guided_attention_in_inference(self, frame_feat, proposal_feat, feat_mask, max_index):
        ...
        index = ops.arange(num_vid).unsqueeze(1)
        query = proposal_feat[index, max_index.t()]
        if feat_mask is not None:
            scores = ops.bmm(key, query.swapaxes(2, 1))
            masked_scores = scores.masked_fill(feat_mask.unsqueeze(-1).equal(0), -1e9)
            masked_scores = nn.Softmax(axis=1)(masked_scores)
            attention_feat = ops.bmm(masked_scores.swapaxes(1, 2), value)
        else:
            scores = ops.bmm(key, query.swapaxes(2, 1))
            scores = nn.Softmax(axis=1)(scores)
            attention_feat = ops.bmm(scores.swapaxes(1, 2), value)
    ...
    def get_pred_from_raw_query(self, query_feat, query_mask, 
        ...
        if return_query_feats:
            ...
            frame_scale_feat = self.key_clip_guided_attention_in_inference(video_feat, video_proposal_feat, video_feat_mask,
                                                                       key_clip_indices)
            frame_scale_scores_ = ops.mul(l2norm(frame_scale_feat),
                                            l2norm(video_query).unsqueeze(0))
            frame_scale_scores = ops.sum(frame_scale_scores_, dim=-1).swapaxes(1, 0)
    ...
    def get_infer_res_from_props_feat(self, modular_query, video_proposal_feat, num_props_for_each_video=10):
        ...
        q2v_topk_scores, q2v_topk_indices = ops.topk(query_context_props_scores.permute((0,2,1)), num_props_for_each_video, dim=-1) # (nq, nv, npev)
    ...
    def get_mixed_infer_res(self, modular_query, clip_proposal_feat, frame_proposal_feat, num_props_for_each_video=10):
        ...
        selected_props_scores, selected_props_indices = ops.topk(mixed_props_scores, num_props_for_each_video, dim=-1)
    ...
    def get_clip_triplet_loss(self, query_context_scores, labels):
        ...
        for i in range(v2t_scores.shape[0]):
            pos_pair_scores = ops.mean(v2t_scores[i][np.where(labels == i)[0].tolist()]) # todo: check


            neg_pair_scores, _ = ops.sort(v2t_scores[i][np.where(labels != i)[0].tolist()], descending=True)
            if self.config.use_hard_negative:
                ...
                sample_neg_pair_scores = neg_pair_scores[
                    ops.randint(0, v2t_sample_max_idx, size=(1,))]

            v2t_loss += (self.config.margin + sample_neg_pair_scores - pos_pair_scores).clamp(min=0).sum()

        # cal_t2v_loss
        text_indices = ops.arange(t2v_scores.shape[0])
        t2v_pos_scores = t2v_scores[text_indices, labels.tolist()]
        mask_score = ops.deepcopy(t2v_scores)
        mask_score[text_indices, labels.tolist()] = 999
        _, sorted_scores_indices = ops.sort(mask_score, descending=True, axis=1)
        ...
        sample_indices = sorted_scores_indices[
            text_indices, ops.randint(1, t2v_sample_max_idx, size=(t2v_scores.shape[0],))]
    ...
    def get_frame_trip_loss(self, query_context_scores):
        ...
        diagonal_indices = ops.arange(bsz)
        pos_scores = query_context_scores[diagonal_indices, diagonal_indices]  # (N, )
        query_context_scores_masked = ops.deepcopy(query_context_scores)
        ...
        neg_query_pos_context_scores = self.get_neg_scores(query_context_scores.swapaxes(0, 1),
                                                           query_context_scores_masked.swapaxes(0, 1))
    ...
    def get_neg_scores(self, scores, scores_masked):
        ...
        bsz = len(scores)
        batch_indices = ops.arange(bsz)

        _, sorted_scores_indices = ops.sort(scores_masked, descending=True, axis=1)
        ...
        # (N, )
        sampled_neg_score_indices = sorted_scores_indices[batch_indices, ops.randint(sample_min_idx, sample_max_idx,
                                                                                       size=(bsz,))]
    ...
    def get_ranking_loss(self, pos_score, neg_score):
        ...
        """
        return ops.clamp(self.config.margin + neg_score - pos_score, min=0).sum() / len(pos_score)
    ...
    def vector_trip_loss(self, pos_sim_score, neg_sim_score, margin):
        ...
        trip_loss = ops.clamp(neg_sim_score - pos_sim_score + margin, min=0)
    ...
    def sum_margin_trip_loss(self, pos_sim_score, neg_sim_score, margin):
        ...
        trip_loss = ops.clamp(neg_sim_score - pos_sim_score + margin, min=0)
        trip_loss = trip_loss.sum(axis=-1)
