...
from utils.basic_utils import AverageMeter, BigFile, read_dict, save_json, save_jsonl, load_json
...
def compute_context_info(model, eval_dataset, opt):
    ...
    column_names = ["clip_video_feature", "frame_video_feature", "index", "video_id"]
    # output_columns = ["clip_videsos", "frame_videos", "videos_mask", "idxs", "video_ids"]
    val_set = GeneratorDataset(eval_dataset, column_names=column_names, shuffle=False, num_parallel_workers=opt.num_workers)
    val_set = val_set.batch(batch_size=opt.eval_context_bsz, num_parallel_workers=opt.num_workers, per_batch_map=collate_frame_val, output_columns=["data"])
    # context_dataloader = val_set.create_dict_iterator(num_epochs=1)
    bsz = opt.eval_context_bsz
    metas = []  # list(dicts)
    clip_feat, frame_feat, frame_mask = [], [], []
    clip_proposal_feat = None
    frame_props_feat = None
    for idx, batch in tqdm(enumerate(val_set), desc="Computing context info",
                           total=len(val_set)):
        batch = batch[0]
        metas.extend([it.item() for it in batch["video_ids"]])
        clip_video_feat_ = batch["clip_videos"]
        frame_video_feat_ = batch["frame_videos"]
        frame_mask_ = batch["videos_mask"]
        ...
        if clip_proposal_feat is None:
            clip_proposal_feat = ops.zeros((n_total_vid, _clip_proposal_feat.shape[1], opt.hidden_size), dtype=_clip_proposal_feat.dtype)
            clip_proposal_feat[idx * bsz:(idx + 1) * bsz] = _clip_proposal_feat
        else:
            clip_proposal_feat[idx * bsz:(idx + 1) * bsz] = _clip_proposal_feat
        # frame props feat
        if  frame_props_feat is None:
            frame_props_feat = ops.zeros((n_total_vid, _frame_props_feat.shape[1], opt.hidden_size), dtype=_frame_props_feat.dtype)
    ...
    def cat_tensor(tensor_list):
        if len(tensor_list) == 0:
            ...
            for i, e in enumerate(tensor_list):
                res_tensor[b_sizes_cumsum[i].item():b_sizes_cumsum[i+1].item(), :seq_l[i]] = e
            return res_tensor
...
def compute_query2ctx_info(model, eval_text_dataset, eval_video_dataset, opt, ctx_info):
    ...
    #                                num_workers=opt.num_workers, shuffle=False, pin_memory=opt.pin_memory)
    column_names = ["cap_tensor", "index", "cap_id"]
    # output_columns = ["target", "words_mask", "idxs", "cap_ids"]
    txt_set = GeneratorDataset(eval_text_dataset, column_names=column_names, num_parallel_workers=opt.num_workers, shuffle=False)
    txt_set = txt_set.batch(batch_size=opt.eval_query_bsz, num_parallel_workers=opt.num_workers, per_batch_map=collate_text_val, output_columns=["data"])
    # query_eval_loader = txt_set.create_dict_iterator(num_epochs=1)
    ...
    score_sum_mssl = []
    vcmr_res = [] # added
    for idx, batch in tqdm(enumerate(txt_set), desc="Computing query2video scores", total=len(txt_set)):
        
        # print("***", idx, "***")
        batch = batch[0]
        _query_metas = [it.item() for it in batch["cap_ids"]]
        query_metas.extend(_query_metas) # changed
        query_feat = batch["target"]
        query_mask = batch["words_mask"]
        ...
        _sorted_scores, _sorted_score_indices = ops.topk(_q2v_topk_scores, max_vcmr_props, dim=1) # ori

        _selected_props_left = _selected_props[:,:,0]
        _selected_props_right = _selected_props[:,:,1]
        _selected_props_left_sorted = ops.gather_elements(_selected_props_left, 1, _sorted_score_indices)
        _selected_props_right_sorted = ops.gather_elements(_selected_props_right, 1, _sorted_score_indices)
        _selected_props_sorted = ops.stack([_selected_props_left_sorted, _selected_props_right_sorted], axis=-1) # [num_query, max_vcmr_props, 2]

        _selected_props_sorted = _selected_props_sorted.asnumpy()
        _sorted_scores = _sorted_scores.asnumpy()
        _sorted_score_indices = _sorted_score_indices.asnumpy()
    ...
    clip_scale_scores = ops.cat(clip_scale_scores, axis=0)
    clip_sorted_indices = ops.argsort(clip_scale_scores, axis=1, descending=True).asnumpy()
    frame_scale_scores = ops.cat(frame_scale_scores, axis=0)
    frame_sorted_indices = ops.argsort(frame_scale_scores, axis=1, descending=True).asnumpy()
    frame_atten_scores = ops.cat(frame_atten_scores, axis=0)
    atten_sorted_indices = ops.argsort(frame_atten_scores, axis=1, descending=True).asnumpy()
    score_sum = ops.cat(score_sum, axis=0)
    sum_sorted_indices = ops.argsort(score_sum, axis=1, descending=True).asnumpy()
    score_sum_mssl = ops.cat(score_sum_mssl, axis=0)
    mssl_sum_sorted_indices = ops.argsort(score_sum_mssl, axis=1, descending=True).asnumpy()
...
def setup_model(opt):
    ...
    checkpoint = ms.load_checkpoint(ckpt_filepath)
    # loaded_model_cfg = checkpoint["model_cfg"] # todo: change
    loaded_model_cfg = load_json(opt.modelcfg_filepath)
    loaded_model_cfg = EDict(loaded_model_cfg)
    # os.environ["CUDA_VISIBLE_DEVICES"] = loaded_model_cfg.device # todo: check
    ...
    model = NAME_TO_MODELS[opt.model_name](loaded_model_cfg) # todo: 处理config参数
    
    param_not_load, _ = ms.load_param_into_net(model, checkpoint)
