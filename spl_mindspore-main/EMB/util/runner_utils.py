...
def save_checkpoint(path, state, is_best, filename='checkpoint.ckpt'):
    fname = os.path.join(path, filename)
    mindspore.save_checkpoint(state, fname)
    bname = os.path.join(path, 'mindspore_best.ckpt')
    if is_best: shutil.copyfile(fname, bname)


def load_checkpoint(path):
    return os.path.join(path, 'mindspore_best.ckpt')


def convert_length_to_mask(lengths, max_len=None):
    max_len = lengths.max().item() if max_len is None else max_len
    mask = P.arange(max_len).broadcast_to((lengths.shape[0], max_len)) < lengths.unsqueeze(1)
    mask = mask.float()
    return mask
...
def calculate_batch_iou(i0, i1):
    ...
    union = (P.minimum(i0[...,0], i1[...,0]), P.maximum(i0[...,1], i1[...,1]))
    inter = (P.maximum(i0[...,0], i1[...,0]), P.minimum(i0[...,1], i1[...,1]))
    iou = 1. * (inter[1] - inter[0]).clamp(min=0) / (union[1] - union[0]).clamp(min=1e-5)
    return iou


def eval_test(model, data_loader, mode='test', epoch=None, global_step=None, elastic=False, max_len=None):
    ious = []
    save_list = []
    iterator = data_loader.create_tuple_iterator(num_epochs=1, do_copy=False)
    for idx, data in tqdm(enumerate(iterator), total=len(data_loader), desc='evaluate {}'.format(mode), leave=False):
        # prepare features
        indexes, vfeats, vfeat_lens, word_ids, char_ids, s_labels, e_labels, _ = data
        records = data_loader.children[0].source.get_record(indexes.asnumpy().tolist())
        # generate mask
        query_mask = (P.zeros_like(word_ids) != word_ids).float()
        video_mask = convert_length_to_mask(vfeat_lens, max_len=max_len)
        # compute predicted results
        (_, start_logits, end_logits, 
         proposals, p_score, p_mask) = model(word_ids, vfeats, video_mask, query_mask)
        if not elastic:
        ...
        start_indices = start_indices.asnumpy()
        end_indices = end_indices.asnumpy()
...
def convert_ela_to_det(start, end, start_labels, end_labels, vmask):
    ...
    cands = cands * (vmask.unsqueeze(-1) * vmask.unsqueeze(1))
    cands = P.triu(cands, diagonal=0)
    cands_start = P.arange(start.shape[1])
    cands_start = P.minimum(cands_start.view(1, -1), lens-1).unsqueeze(-1)
    cands_end = P.arange(end.shape[1])
    cands_end = P.minimum(cands_end.view(1, -1), lens-1).unsqueeze(1) + 1
    cands_bound = P.stack([cands_start.repeat(1, 1, end.shape[1]), 
                                cands_end.repeat(1, start.shape[1], 1)], 
                                axis=-1).view(cands.shape[0], -1, 2) # Nx(128x128)x2
    # ious between all 
    moments = P.stack([start_labels, end_labels+1], axis=1)
    ...
    _, start_index = P.max(P.max(ious, axis=2)[0], axis=1)  # (batch_size, )
    _, end_index = P.max(P.max(ious, axis=1)[0], axis=1)  # (batch_size, )
