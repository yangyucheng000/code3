import copy
import mindspore
import mindspore.nn as nn
import mindspore.ops as P

from model.layers import Embedding, VisualProjection, downscale1d
from model.layers import ConditionedPredictor, HighLightLayer
from model.encoder import GuidedAttentionEncoder
from model.predictor import LSTMBlock
from model.fuser import VSLFuser
from model.position import CosinePosition
from model.proposal import TAN2dProposal
from model.ranker import Conv2dRanker
from mindspore.common.initializer import initializer

from util.runner_utils import calculate_batch_iou
from util.lr_scheduler import LinearWithWarmUpLR


def build_optimizer_and_scheduler(model, configs):
    no_decay = ['bias', 'layer_norm', 'LayerNorm', 'beta']  # no decay for parameters of layer norm and bias
    optimizer_grouped_parameters = [
        {'params': [p for p in model.trainable_params() if not any(nd in p.name for nd in no_decay)],
         'weight_decay': 0.01},
        {'params': [p for p in model.trainable_params() if any(nd in p.name for nd in no_decay)], 'weight_decay': 0.0}]
    scheduler = LinearWithWarmUpLR(learning_rate=configs.init_lr, total_steps=configs.num_train_steps, warmup_ratio=configs.warmup_proportion)
    optimizer = nn.AdamWeightDecay(optimizer_grouped_parameters, learning_rate=scheduler)
    return optimizer


class EMB(nn.Cell):
    def __init__(self, configs, word_vectors):
        super(EMB, self).__init__()
        self.configs = configs
        self.embedding_net = Embedding(num_words=configs.word_size, num_chars=configs.char_size, out_dim=configs.dim,
                                       word_dim=configs.word_dim, char_dim=configs.char_dim, word_vectors=word_vectors,
                                       drop_rate=configs.drop_rate)
        self.video_affine = VisualProjection(visual_dim=configs.video_feature_dim, dim=configs.dim,
                                             drop_rate=configs.drop_rate)
        # positional encoding
        self.position = CosinePosition(**vars(configs))
        # encoder
        params = dict(**vars(configs))
        params['proposal'] = TAN2dProposal(**vars(configs))
        self.encoder = GuidedAttentionEncoder(**params)
        # fuser
        self.fuser1 = VSLFuser(**vars(configs))
        self.fuser2 = VSLFuser(**vars(configs))
        # query-guided highlighting
        self.highlight_layer = HighLightLayer(dim=configs.dim)
        # conditioned predictor
        params = dict(**vars(configs))
        params['num_layers'], params['drop_rate'] = 1, 0
        self.predictor = ConditionedPredictor(configs.dim, LSTMBlock(**params))
        # ranker
        self.ranker = Conv2dRanker(**vars(configs))
        self.range_constant = P.arange(configs.max_pos_len)
        # init parameters
        self.init_parameters()

    def init_parameters(self):
        for name, cell in self.cells_and_names():
            if isinstance(cell, nn.Conv2d) or isinstance(cell, nn.Conv1d) or isinstance(cell, nn.Dense):
                cell.weight.set_data(initializer('xavier_uniform', cell.weight.shape, cell.weight.dtype))
                if cell.bias is not None:
                    cell.bias.set_data(initializer('zeros', cell.bias.shape, cell.bias.dtype))

    def construct(self, word_ids, video_features, v_mask, q_mask, moments=None):
        video_features = self.video_affine(video_features)
        query_features = self.embedding_net(word_ids)
        video_features, query_features = self.position(video_features), self.position(query_features)
        ((video_features, vquery_features), 
         (proposal_features, pquery_features), proposals, p_mask) = self.encoder(vfeats=video_features, 
                            qfeats=query_features, vmask=v_mask, qmask=q_mask,
                            moments=moments)
        # conduct frame-level regression
        features = self.fuser1(vfeats=video_features, qfeats=vquery_features,
                            vmask=v_mask, qmask=q_mask)
        h_score = self.highlight_layer(features, v_mask)
        features = features * h_score.unsqueeze(2)
        start_logits, end_logits = self.predictor(features, mask=v_mask)
        # conduct proposal level ranking
        features = self.fuser2(vfeats=proposal_features, qfeats=pquery_features,
                            vmask=p_mask, qmask=q_mask)
        # convert proposal's feature and mask back to 2d for ranking
        p_score = self.ranker(features, p_mask)
        # flatten proposal's bounds, mask and scores as outputs
        return h_score, start_logits, end_logits, proposals, p_score, p_mask

    def compute_ranking_loss(self, moments, proposals, scores, mask, min_iou, max_iou, noise_alpha=0.75, noise_beta=0.75, use_reweight=False):
        #TODO
        # return 0.0, None, None
        return self.ranker.compute_loss(
            moments=moments, bounds=proposals, 
            scores=scores, mask=mask, min_iou=min_iou, max_iou=max_iou, use_reweight=use_reweight)

    def compute_highlight_loss(self, scores, labels, mask, moments, 
                    proposals, pscores, pmask, threshold=0.5, extend=0.1, sample_weight=None):
        # get the most cconfident proposals as the pseudo labels
        pseudo = self.ranker.topk_confident(P.stack([proposals[...,0], proposals[...,1]+1], axis=-1), 
            pscores, pmask, P.stack([moments[...,0], moments[...,1]+1], axis=-1), threshold, k=1).squeeze(1)
        # start/end: (<bsz>)
        start = P.minimum(moments[:,0], pseudo[:,0])
        end = P.minimum(moments[:,1], pseudo[:,1])
        # extend the pseudo boundary: start/end (<bsz>)
        extension = ((end - start + 1).float() * extend).round().long()
        start = (start - extension).clamp(min=0)
        end = P.minimum(mask.sum(-1).long() - 1, end + extension)
        # build the frame-wise binary labels: (<bsz>, <#frames>)
        labels = self.range_constant.view(1, -1)
        labels = (labels >= start.view(-1, 1)).float() * (labels <= end.view(-1, 1)).float()
        labels = P.stop_gradient(labels).long()
        loss = self.highlight_layer.compute_loss(scores=scores, labels=labels, mask=mask, sample_weight=sample_weight)
        return loss

    def compute_loss(self, start_logits, end_logits, start_labels, end_labels,
            proposals, pscores, pmask, threshold=0.5, sample_weight=None):
        moments = P.stack([start_labels, end_labels], axis=1)
        # get the most confident proposals as the pseudo labels
        pseudo = self.ranker.topk_confident(P.stack([proposals[...,0], proposals[...,1]+1], axis=-1), 
            pscores, pmask, P.stack([moments[...,0], moments[...,1]+1], axis=-1), threshold, k=1).squeeze(1)
        # construct labels for the start and end logits
        start = self.range_constant.view(1, -1)
        start = ((start >= P.minimum(moments[:,0], pseudo[:,0]).view(-1, 1)).float() *
                 (start <= P.maximum(moments[:,0], pseudo[:,0]).view(-1, 1)).float())
        end = self.range_constant.view(1, -1)
        end = ((end >= P.minimum(moments[:,1], pseudo[:,1]).view(-1, 1)).float() *
               (end <= P.maximum(moments[:,1], pseudo[:,1]).view(-1, 1)).float())
        start = P.stop_gradient(start)
        end = P.stop_gradient(end)
        # maximise the logit of frames falling in the candidated spans
        start_loss = (P.softmax(start_logits, axis=-1) * start).sum(-1).log()
        end_loss = (P.softmax(end_logits, axis=-1) * end).sum(-1).log()
        if sample_weight is not None:
            sample_weight = sample_weight.squeeze(-1)
            loss_scale = P.stop_gradient(start_loss.sum() / ((start_loss * sample_weight).sum() + 1e-9))
            start_loss = start_loss * sample_weight * loss_scale
            loss_scale = P.stop_gradient(end_loss.sum() / ((end_loss * sample_weight).sum() + 1e-9))
            end_loss = end_loss * sample_weight * loss_scale
        start_loss = -start_loss.mean()
        end_loss = -end_loss.mean()
        return start_loss + end_loss

    def extract_index(self, start_logits, end_logits, proposals, pscores, pmask):
        # get the most confident proposals as the pseudo labels
        ppseudo = self.ranker.topk_confident(P.stack([proposals[...,0], proposals[...,1]+1], axis=-1), 
            pscores, pmask, None, 0., k=1).squeeze(1)
        bpseudo = P.stack(self.predictor.extract_index(start_logits, end_logits), axis=-1)
        # construct labels for the start and end logits
        start = self.range_constant.view(1, -1)
        start = ((start >= P.minimum(bpseudo[:,0], ppseudo[:,0]).view(-1, 1)) *
                 (start <= P.maximum(bpseudo[:,0], ppseudo[:,0]).view(-1, 1)))
        end = self.range_constant.view(1, -1)
        end = ((end >= P.minimum(bpseudo[:,1], ppseudo[:,1]).view(-1, 1)) *
               (end <= P.maximum(bpseudo[:,1], ppseudo[:,1]).view(-1, 1)))
        # assert (start.sum(-1) >= 1).all() and (end.sum(-1) >= 1).all()
        return start, end
    