...
class FixedPosition(nn.Cell):
    ...
    def __init__(self, dim=128, max_pos_len=128, pe_type="cosine"):
        ...
        if pe_type != "none":
            position = P.arange(0, max_pos_len).float().unsqueeze(1)
            if pe_type == "cosine":
                # Compute the positional encodings once in log space.
                pe = P.zeros((max_pos_len, dim))  # (L, D)
                div_term = P.exp(P.arange(0, dim, 2).float() * - (math.log(10000.0) / dim))
            ...
            self.pe = mindspore.Parameter(pe, name='pe', requires_grad=False)
    ...
    def construct(self, x):
        ...
        if self.pe_type != "none":
            pe = self.pe[:x.shape[-2], :]  # (#x.size(-2), n_filters)
            extra_dim = len(x.shape) - 2
